...
# Define Models
class OfficeHomeCNN(BaseModelArchitecture):
    def __init__(self, num_classes=65):
        super(OfficeHomeCNN, self).__init__()
...
class OfficeHomeSkipNet(BaseModelArchitecture):
    def __init__(self, num_classes=65):
        super(OfficeHomeSkipNet, self).__init__()
        ...
        self.stage1 = nn.Sequential(
            BottleneckBlock(64, 32, 64, stride=1),
            BottleneckBlock(64, 32, 64, stride=1)
        )
        self.stage2 = nn.Sequential(
            BottleneckBlock(64, 64, 128, stride=2),
            BottleneckBlock(128, 64, 128, stride=1)
        )
        self.stage3 = nn.Sequential(
            BottleneckBlock(128, 128, 256, stride=2),
            BottleneckBlock(256, 128, 256, stride=1)
        )
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
...
def print_debug_info(agent, num_epochs, loss, accuracy_fn):
    ...
    domain_counts = {domain: agent.len_domain_samples(domain) for domain in ['RealWorld', 'Clipart', 'Product', 'Art']}
...
def main():
    ...
    parser.add_argument('--models', type=str, nargs='+', default=['OfficeHomeCNN'],
