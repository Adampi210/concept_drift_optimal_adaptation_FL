...
experiment_filename='evaluate_policy_MEMD-ABSA.py'
...
for current_seed in "${seeds_to_run[@]}"; do
    ...
    # Construct a unique log filename including model name, current_seed, and timestamp
    log_file="$log_dir/drift_src_${src_log_str}_model_${MODEL_NAME}_policy_${POLICY_ID}_setting_${SETTING_ID}_schedule_${SCHEDULE}_seed_${current_seed}_${timestamp}.log"
    ...
    LD_LIBRARY_PATH="" python3 "$experiment_filename" \
