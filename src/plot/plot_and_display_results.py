...
def compare_policies(setting_id, schedule_type, source_domain='photo', target_domain='sketch', 
                    policy_ids=[1, 2, 3, 4], model_name='PACSCNN', results_dir='../../data/results/', T=None):
    """
    ...
    Args:
        ...
        schedule_type (str): Type of drift schedule (e.g., 'burst_0', 'oscillating_0').
        source_domain (str): Source domain name.
        target_domain (str): Target domain name.
        policy_ids (list): List of policy IDs to compare.
        model_name (str): Model architecture name (e.g., 'PACSCNN').
    ...
    """
    print(f"Comparing policies for Setting ID: {setting_id}, Schedule: {schedule_type}")
    ...
    for policy_id, color in zip(policy_ids, colors):
        ...
        # Aggregate data across all matching files (seeds)
        for file_path in matching_files:
        ...
        # Sort epochs and optionally truncate to time T
        epochs = sorted(combined_data.keys())
        if T is not None:
        ...
        # Add decision markers on drift rate plot where updates occur
    ...
    for policy_id in policy_ids:
        ...
        if matching_files:
            ...
            for file_path in matching_files:
                ...
                if T is not None:
                    accs = accs[:T+1]  # +1 because T is inclusive
            ...
            update_rate = np.sum(all_updates) / len(all_updates)  # Average updates per round
