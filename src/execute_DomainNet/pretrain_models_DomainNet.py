...
def train_model(model_class, model_path, seed, domain, num_epochs, batch_size, learning_rate, optimizer_choice, results_save_path, img_size):
    ...
    root_dir = f'/scratch/gautschi/apiasecz/data/DomainNet/' ### directory with the DomainNet dataset
    full_dataset = DomainNetDataHandler(root_dir=root_dir, transform=train_transform).dataset
    ...
    # Create subsets
    train_data_handler = DomainNetDataHandler(root_dir=root_dir, transform=train_transform)
    train_data_handler.set_subset(train_indices)
    holdout_data_handler = DomainNetDataHandler(root_dir=root_dir, transform=holdout_transform)
...
# Main Function
def main():
    parser = argparse.ArgumentParser(description="Pretrain Models on DomainNet Dataset Single Domains")
    parser.add_argument('--seed', type=int, default=0, help='Base random seed')
    parser.add_argument('--model', type=str, default='DomainNetNet', choices=['DomainNetNet',],
                        help='Models to train')
    parser.add_argument('--domains', type=str, nargs='+', default=['real', 'clipart', 'painting', 'sketch'],
