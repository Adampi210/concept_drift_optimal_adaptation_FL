...
# Define Models
class DomainNetNet(BaseModelArchitecture):
    def __init__(self, num_classes=345):  # Changed from 65
        super(DomainNetNet, self).__init__()
        self.model = _resnet50(pretrained=True)
        ...
        # Freeze conv1, bn1, layer1, layer2, layer3 (same as before)
...
class DriftScheduler:
    SCHEDULE_CONFIGS = {
        "burst": lambda: {
            ...
            'base_rate': 0.0,
            'burst_rate': 0.4,
            'target_domains': ['real', 'sketch', 'painting'],
            'initial_delay': 45,
            'strategy': 'replace'
        },
        "spikes": lambda: {
            ...
            'base_rate': 0.0,
            'burst_rate': (0.3, 0.6),
            'target_domains':  ['real', 'sketch', 'clipart'],
        ...
        "step": lambda: {
            ...
            'step_domains': ['clipart', 'sketch', 'painting', 'real', 'clipart'],
            'strategy': 'replace'
        },
        "constant": lambda: {
            'drift_rate': 0.016,
            'domain_change_interval': 50,
            'target_domains': ['clipart', 'real', 'sketch', 'painting'],
            'strategy': 'replace'
        },
        "wave": lambda: {
            'burst_interval': 70,
        ...
        "decaying_spikes": lambda: {
            ...
            'base_rate': 0.0,
            'burst_rate': 0.35,
            'target_domains': ['clipart', 'real', 'sketch'],
            'initial_delay': 20,
            'strategy': 'replace'
        },
...
def evaluate_policy_under_drift(
    ...
    root_dir = f'/scratch/gautschi/apiasecz/data/DomainNet/'
    full_dataset = DomainNetDataHandler(root_dir=root_dir, transform=transform).dataset
    ...
    # Create subsets
    train_data_handler = DomainNetDataHandler(root_dir=root_dir, transform=transform)
    train_data_handler.set_subset(train_indices)
    holdout_data_handler = DomainNetDataHandler(root_dir=root_dir, transform=transform)
...
settings = {
        ...
        60: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.5, 'K_d': 2.0, 'lr': 0.05, 'n_steps':5},
        61: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 2.0, 'lr': 0.05, 'n_steps':5},
        62: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 3.0, 'K_d': 2.0, 'lr': 0.05, 'n_steps':5},
        63: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 5.0, 'K_d': 2.0, 'lr': 0.05, 'n_steps':5},
        64: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 10.0, 'K_d': 2.0, 'lr': 0.05, 'n_steps':5},
        65: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.5, 'K_d': 1.0, 'lr': 0.05, 'n_steps':5},
        66: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 1.0, 'lr': 0.05, 'n_steps':5},
        67: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 3.0, 'K_d': 1.0, 'lr': 0.05, 'n_steps':5},
        68: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 5.0, 'K_d': 1.0, 'lr': 0.05, 'n_steps':5},
        69: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 10.0, 'K_d': 1.0, 'lr': 0.05, 'n_steps':5},
        70: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':30},
        71: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.35, 'lr': 0.05, 'n_steps':30},
        72: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.25, 'lr': 0.05, 'n_steps':30},
        73: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.8, 'K_d': 0.5, 'lr': 0.05, 'n_steps':30},
        74: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.8, 'K_d': 0.35, 'lr': 0.05, 'n_steps':30},
        75: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.1, 'K_d': 0.1, 'lr': 0.1, 'n_steps':70},
        76: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.1, 'lr': 0.05, 'n_steps':5},
        77: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.75, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        78: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.75, 'K_d': 0.1, 'lr': 0.05, 'n_steps':5},
        79: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
    }
DSET_SIZE = 8192 * 2
...
def main():
    ...
    parser = argparse.ArgumentParser(description="DomainNet Evaluation with Dynamic Drift")
    ...
    parser.add_argument('--src_domains', type=str, nargs='+', default=['real',])
    ...
    parser.add_argument('--model_name', type=str, default='DomainNetNet', choices=['DomainNetNet',], help='Model architecture to use')
    parser.add_argument('--img_size', type=int, default=224, help='Size to resize images to (img_size x img_size)')
    args = parser.parse_args()
    
    # Model selection
    model_architectures = {
        'DomainNetNet': DomainNetNet,
    }
    print(f'Model used: {args.model_name}')
    ### MODIFY IF STORED IN A DIFFERENT LOCATION
    model_path = f"/scratch/gautschi/apiasecz/models/concept_drift_models/{args.model_name}_{'_'.join(args.src_domains)}_img_{args.img_size}_seed_{args.seed}.pth"
