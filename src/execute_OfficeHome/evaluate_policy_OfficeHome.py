...
class DriftScheduler:
    SCHEDULE_CONFIGS = {
        "burst": lambda: {
            ...
            'base_rate': 0.0,
            'burst_rate': 0.4,
            'target_domains': ['RealWorld', 'Product', 'Clipart'],
            'initial_delay': 45,
            'strategy': 'replace'
        },
        "spikes": lambda: {
        ...
        "step": lambda: {
            ...
            'step_domains': ['Clipart', 'Product', 'Art', 'RealWorld', 'Clipart'],
            'strategy': 'replace'
        },
        "constant": lambda: {
            'drift_rate': 0.016,
            'domain_change_interval': 50,
            'target_domains': ['Clipart', 'RealWorld', 'Product', 'Art'],
            'strategy': 'replace'
        },
        "wave": lambda: {
            'burst_interval': 70,
        ...
        "decaying_spikes": lambda: {
            ...
            'base_rate': 0.0,
            'burst_rate': 0.35,
            'target_domains': ['Clipart', 'RealWorld', 'Product'],
            'initial_delay': 20,
            'strategy': 'replace'
        },
...
def evaluate_policy_under_drift(
    ...
    ### ROOT DIRECTORY with OfficeHome dataset, replce with custom
    root_dir = f'/scratch/gautschi/apiasecz/data/OfficeHome/OfficeHomeDataset_10072016/'
    full_dataset = OfficeHomeDataHandler(root_dir=root_dir, transform=transform).dataset
    ...
    # Create subsets
    train_data_handler = OfficeHomeDataHandler(root_dir=root_dir, transform=transform)
    train_data_handler.set_subset(train_indices)
    holdout_data_handler = OfficeHomeDataHandler(root_dir=root_dir, transform=transform)
...
settings = {
        ...
        70: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.5, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        71: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 1.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        72: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 3.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        73: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 5.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        74: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 10.0, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
        75: {'pi_bar': 0.1, 'V': 10, 'L_i': 0, 'K_p': 0.5, 'K_d': 0.5, 'lr': 0.05, 'n_steps':5},
...
def main():
    ...
    parser = argparse.ArgumentParser(description="OfficeHomenet Evaluation with Dynamic Drift")
    ...
    parser.add_argument('--src_domains', type=str, nargs='+', default=['RealWorld',])
    ...
    parser.add_argument('--model_name', type=str, default='OfficeHomeNet', choices=['OfficeHomeNet',], help='Model architecture to use')
    parser.add_argument('--img_size', type=int, default=224, help='Size to resize images to (img_size x img_size)')
    args = parser.parse_args()
    
    # Model selection
    model_architectures = {
        'OfficeHomeNet': OfficeHomeNet,
    }
